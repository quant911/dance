/* lmder.f -- translated by f2c (version 19980516).
...
/* Subroutine */ int lmderBound_(S_fp fcn, integer *m, integer *n, doublereal *x, 
...
/*	 lb, ub are arrays that store the upper and lower bounds for each dimension. */
...
/*     ********** */
    ...
    /* NEW CHANGE: Parameter adjustments for lb, ub */
    --lb;
    --ub;
...
/*           store the direction p and x + p. calculate the norm of p. */
    ...
    for (j = 1; j <= i__1; ++j) {
	wa1[j] = -wa1[j];
	/* NEW CHANGE:  Adjust for boundary checks. */
        if (x[j] + wa1[j] < lb[j]) 
		wa1[j] = lb[j]-x[j];
	if (x[j] + wa1[j] > ub[j])
		wa1[j] = ub[j]-x[j];
